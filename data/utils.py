...
from .get_util import get_logger, get_class
...
class FilterGraySliceBuilder(SliceBuilder):
    """
    Filter patches containing more than `1 - threshold` of ignore_index label
    """

    def __init__(self, raw_dataset, label_dataset, weight_dataset, patch_shape, stride_shape, gray_threshold=100, gray_percentile=0.2, **kwargs):
        ...
        def ignore_predicate(raw_label_idx):
            ...
            if gray_percentile != 0:
                raw = np.copy(raw_dataset[label_idx])
                gray_counts = np.count_nonzero(raw > gray_threshold)
                gray_counts = gray_counts / raw.size
                accept_gray = gray_counts >= gray_percentile
            else:
                accept_gray = True
            return accept_gray

        # zipped_slices = zip(self.raw_slices, self.label_slices)
        zipped_slices = list(self.raw_slices)
        # ignore slices containing too much ignore_index
        filtered_slices = list(filter(ignore_predicate, zipped_slices))
        raw_slices = filtered_slices
...
class FilterSliceBuilder(SliceBuilder):
    ...
    def __init__(self, raw_dataset, label_dataset, weight_dataset, patch_shape, stride_shape, ignore_index=(0,),
                 threshold=0.6, slack_acceptance=0.01, gray_threshold=200, gray_percentile=0.2, **kwargs):
        super().__init__(raw_dataset, label_dataset, weight_dataset, patch_shape, stride_shape, **kwargs)
        if label_dataset is None:
            return
        ...
        def ignore_predicate(raw_label_idx):
            ...
            if threshold != 0:
                ...
                accept_label = non_ignore_counts >= threshold
            else:
                accept_label = True
            return (accept_label and accept_gray) or rand_state.rand() < slack_acceptance
...
def get_test_loaders(config):
    ...
    # get dataset class
    if config.loaders.dataset_mode == 'cmsr':
        dataset_cls_str = 'StandardHDF5Dataset'
    elif config.loaders.dataset_mode == 'cmsrnii':
        dataset_cls_str = 'StandardNIIDataset'
    elif config.loaders.dataset_mode == 'cmsreval':
        dataset_cls_str = 'StandardCmsrEvalDataset'
    elif config.loaders.dataset_mode == 'provo':
        dataset_cls_str = 'StandardProvoDataset'
    dataset_class = _loader_classes(dataset_cls_str)

    test_datasets = dataset_class.create_datasets(loaders_config, phase='test')

    num_workers = loaders_config.get('num_threads', 1)
