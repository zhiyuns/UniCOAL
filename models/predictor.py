        ...
        with torch.no_grad():
            for i, (batch, extra_raw, indices) in enumerate(test_loader):
                ...
                if output_heads == 3:
                    ...
                    if hasattr(self.model, 'pred_mask'):
                        prediction_label = self.model.pred_mask
                    else:
                        prediction_label = torch.zeros(self.model.fake_B.shape)
                    if len(prediction_label.size()) == 4:
                        prediction_label = prediction_label.unsqueeze(1)
                    if prediction_label.size()[1] > 1:
                        prediction_label = torch.argmax(prediction_label, dim=1, keepdim=True)
