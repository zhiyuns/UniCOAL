...
class Conv3dLayer(torch.nn.Module):
    def __init__(self,
        ...
        self.weight_gain = 1 / np.sqrt(in_channels * (kernel_size[0] * kernel_size[1] * kernel_size[2]))
        self.act_gain = bias_act.activation_funcs[activation].def_gain

        memory_format = torch.channels_last if channels_last else torch.contiguous_format
        weight = torch.randn([out_channels, in_channels, kernel_size[0], kernel_size[1], kernel_size[2]]).to(memory_format=memory_format)
        bias = torch.zeros([out_channels]) if bias else None
        if trainable:
            self.weight = torch.nn.Parameter(weight)
            self.bias = torch.nn.Parameter(bias) if bias is not None else None
    ...
    def forward(self, x, gain=1):
        w = self.weight * self.weight_gain
        b = self.bias.to(x.dtype) if self.bias is not None else None
        x = torch.nn.functional.conv3d(input=x, weight=w.to(x.dtype), bias=None, stride=self.stride, padding=self.padding)
...
class MappingNetwork(torch.nn.Module):
    def __init__(self,
        ...
        **kwargs
        ...
        if index_dim != 0:
            if posi_emb == 'linear':
                # self.posi_emb = FullyConnectedLayer(index_dim, embed_features_index)
                self.posi_emb = None
                embed_features_index = 1
            elif posi_emb == 'fourier':
                self.posi_emb = FixedTimeEncoder(128, 1, True)
                embed_features_index = self.posi_emb.get_dim()
            self.embed_index = FullyConnectedLayer(embed_features_index, embed_features)
    ...
    def forward(self, z, c, index, truncation_psi=1, truncation_cutoff=None, skip_w_avg_update=False, **kwargs):
...
def construct_log_spaced_freqs(max_num_frames: int, skip_small_t_freqs: int=0, normed=False):
...
class FixedTimeEncoder(torch.nn.Module):
    def __init__(self,
        ...
        fourier_coefs = construct_log_spaced_freqs(max_num_frames, skip_small_t_freqs=skip_small_t_freqs, normed=normed)
