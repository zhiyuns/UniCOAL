...
class CoModDiscriminator(torch.nn.Module):
    def __init__(self,
        ...
        **kwargs
        ...
        if c_dim > 0:
            self.mapping = MappingNetwork(z_dim=0, c_dim=c_dim, index_dim=index_dim, w_dim=cmap_dim, num_ws=None, w_avg_beta=None, **mapping_kwargs)
        self.b4 = DiscriminatorEpilogue(channels_dict[4], cmap_dim=cmap_dim, resolution=4, **epilogue_kwargs, **common_kwargs)

    def forward(self, img, c, index, **block_kwargs):
        x = None
        for res in self.block_resolutions:
            block = getattr(self, f'b{res}')
        ...
        cmap = None
        if self.c_dim > 0:
            cmap = self.mapping(None, c, index)
