...
class upBlock(nn.Module):
    def __init__(self, in_channels, skip_channels, out_channels):
        ...
        self.conv = double_conv(skip_channels + in_channels, out_channels)

    def forward(self, x1, x2):  # x1是需要上采样的图，x2为经过对应大小的下采样图
        deX = self.up(x1)  # 原图大小乘二
        x = deX
        x = torch.cat([x2, deX], dim=1)  # 与对应的下采样图拼接
        x = self.conv(x)
...
class ResidualConv(nn.Module):
    def __init__(self, input_dim, output_dim, kernel, stride, padding):
        ...
        self.conv_block = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Conv3d(output_dim, output_dim, kernel_size=kernel, padding=1),
        )
        self.conv_skip = nn.Sequential(
            nn.Conv3d(input_dim, output_dim, kernel_size=kernel, stride=stride, padding=1),
            nn.BatchNorm3d(output_dim),
        )
    ...
    for i, out_feature_num in enumerate(f_maps):
        if i == 0:
            ...
            encoder = Encoder(f_maps[i - 1], out_feature_num,
                              apply_pooling=False,  # skip pooling in the last level
                              basic_module=basic_module,
                              conv_layer_order=layer_order,
                              conv_kernel_size=conv_kernel_size,
                              num_groups=num_groups,
